...
<?php
			...
			if(isset($_POST['etiquetar'])){
				$agente	 			= mysqli_real_escape_string($connect,(strip_tags($_POST["agente"],ENT_QUOTES)));//Escanpando caracteres 
					...
					$update = mysqli_query($connect, "UPDATE sales SET name_client='$nameClient', lastname_client='$lastNameClient', phone='$phone', phone_alt='$phoneAlt', gender='$genClient', age='$ageClient', address='$direccion', zip_code='$zipcode', zip_4='$zipfour', city='$city', state='$state', payment_type='$payment', procesadora='$cardProcess', coment_payment='$comentPayment', price='$price', re_order='$reOrder', coment='$coment', product1='$productOne',  qty1='$qtyOne', product2='$productTwo', qty2='$qtyTwo', product3='$productThree', qty3='$qtyThree', product4='$productFour', qty4='$qtyFour', product5='$productFive', qty5='$qtyFive', date_edit='$fechaEdit', status='$status', valid_post='$fechaEdit', valid_for='$usuarioModif' WHERE id='$nik'") or die(mysqli_error());
					if($update){
					header("Location: ../views/index-table-order-postf.php");
				}else{
			...
			if(isset($_POST['modificate'])){
				$agente	 			= mysqli_real_escape_string($connect,(strip_tags($_POST["agente"],ENT_QUOTES)));//Escanpando caracteres 
					...
					if($update){
						header("Location: action-edit-order-postf.php?nik=".$nik."&pesan=sukses");
					}else{
						echo '<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Error, no se pudo guardar los datos.</div>';
					}
				}	
                    ...
                    <select class="form-control" name="product5">
                      ...
                      <input type="text" name="qty5" maxlength="2" value="<?php echo $row ['qty5'] ?>" class="form-control" placeholder="Cantidad">
                        ...
                        <span class="input-group-text">
						  ...
						  <div class="form-group">
                    <div class="input-group">
                    <input type="text" name="statusOrder" value="<?php echo $row ['status_approval']; ?>" class="form-control" readonly="readonly">
					            <div class="input-group-append">
                        <span class="input-group-text">
                          <i class="mdi mdi-check-circle-outline"></i>
                        </span>
