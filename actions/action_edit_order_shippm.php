...
<?php
			...
			if(isset($_POST['modificate'])){
				...
				$comentPaymentNew	= mysqli_real_escape_string($connect,(strip_tags($_POST["comentPayment"],ENT_QUOTES)));//Escanpando caracteres$price	     		= mysqli_real_escape_string($connect,(strip_tags($_POST["priceClient"],ENT_QUOTES)));//Escanpando caracteres 
				...
				$update = mysqli_query($connect, "UPDATE sales SET name_client='$nameClient', lastname_client='$lastNameClient', phone='$phone', phone_alt='$phoneAlt', gender='$genClient', age='$ageClient', address='$direccion', zip_code='$zipcode', zip_4='$zipfour', city='$city', state='$state', payment_type='$payment', procesadora='$cardProcess', coment_payment='$comentPayment', price='$price', re_order='$reOrder', coment='$coment', product1='$productOne',  qty1='$qtyOne', product2='$productTwo', qty2='$qtyTwo', product3='$productThree', qty3='$qtyThree', product4='$productFour', qty4='$qtyFour', product5='$productFive', qty5='$qtyFive', date_edit='$fechaEdit', coment='$coment' WHERE id='$nik'") or die(mysqli_error());
				if($update){
					header("Location: action-edit-order-shippm.php?nik=".$nik."&pesan=sukses");
				}else{
