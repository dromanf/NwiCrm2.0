...
<?php
			...
			if(mysqli_num_rows($sql) == 0){
				header("Location: ../views/index-table-order-pent-send.php");
			}else{
			...
			if(isset($_POST['alertate'])){
				$agente	 			= mysqli_real_escape_string($connect,(strip_tags($_POST["agente"],ENT_QUOTES)));//Escanpando caracteres 
					...
					$update = mysqli_query($connect, "UPDATE db_sales SET name_client='$nameClient', lastname_client='$lastNameClient', phone='$phone', phone_alt='$phoneAlt', gender='$genClient', age='$ageClient', address='$direccion', zip_code='$zipcode', zip_4='$zipfour', city='$city', state='$state', payment_type='$payment', procesadora='$cardProcess', coment_payment='$comentPayment', price='$price', re_order='$reOrder', coment='$coment', product1='$productOne',  qty1='$qtyOne', product2='$productTwo', qty2='$qtyTwo', product3='$productThree', qty3='$qtyThree', product4='$productFour', qty4='$qtyFour', product5='$productFive', qty5='$qtyFive', date_edit='$fechaEdit', status='$status', date_alert='$fechaEdit', alert_for='$usuarioModif' WHERE id='$nik'") or die(mysqli_error());
					if($update){
					header("Location: ../views/index-table-order-pent-send.php");
				}else{
					echo '<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Error, no se pudo guardar los datos.</div>';
			...
			if(isset($_POST['modificate'])){
					...
					if($update){
						header("Location: action-edit-order-send.php?nik=".$nik."&pesan=sukses");
					}else{
						echo '<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Error, no se pudo guardar los datos.</div>';
					}
				}
