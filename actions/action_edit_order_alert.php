...
<?php
			...
			if(mysqli_num_rows($sql) == 0){
				header("Location: ../views/fronts/table_order_alert.php");
			}else{
			...
			if(isset($_POST['etiquetar'])){
				$agente	 			= mysqli_real_escape_string($connect,(strip_tags($_POST["agente"],ENT_QUOTES)));//Escanpando caracteres 
					...
					if($update){
					header("Location: .../fronts/table_order_alert.php");
				}else{
          ...
          $price	     		= mysqli_real_escape_string($connect,(strip_tags($_POST["priceClient"],ENT_QUOTES)));//Escanpando caracteres 
					...
					if($update){
					header("Location: ../views/fronts/table-order-postf.php");
				}else{
					echo '<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Error, no se pudo guardar los datos.</div>';
				}
			}
			if(isset($_POST['modificate'])){
				$agente	 			= mysqli_real_escape_string($connect,(strip_tags($_POST["agente"],ENT_QUOTES)));//Escanpando caracteres 
					...
					if($update){
						header("Location: action_edit_order_alert.php?nik=".$nik."&pesan=sukses");
					}else{
						echo '<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Error, no se pudo guardar los datos.</div>';
					}
				}					
