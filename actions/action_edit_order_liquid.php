...
<?php
			...
			if(mysqli_num_rows($sql) == 0){
				header("Location: ../views/fronts/table_order_paid.php");
			}else{
			...
			if(isset($_POST['etiquetar'])){
				$agente	 			= mysqli_real_escape_string($connect,(strip_tags($_POST["agente"],ENT_QUOTES)));//Escanpando caracteres 
					...
					$update = mysqli_query($connect, "UPDATE db_sales SET name_client='$nameClient', lastname_client='$lastNameClient', phone='$phone', phone_alt='$phoneAlt', gender='$genClient', age='$ageClient', address='$direccion', zip_code='$zipcode', zip_4='$zipfour', city='$city', state='$state', payment_type='$payment', procesadora='$cardProcess', coment_payment='$comentPayment', price='$price', re_order='$reOrder', coment='$coment', product1='$productOne',  qty1='$qtyOne', product2='$productTwo', qty2='$qtyTwo', product3='$productThree', qty3='$qtyThree', product4='$productFour', qty4='$qtyFour', product5='$productFive', qty5='$qtyFive', date_edit='$fechaEdit', status='$status', date_paid='$fechaEdit', paid_for='$usuarioModif' WHERE id='$nik'") or die(mysqli_error());
					if($update){
					header("Location: ../views/fronts/table_order_paid.php");
				}else{
			...
			if(isset($_POST['VolverStat'])){
				$agente	 			= mysqli_real_escape_string($connect,(strip_tags($_POST["agente"],ENT_QUOTES)));//Escanpando caracteres 
					...
					if($update){
					header("Location: ../views/fronts/table_order_postf.php");
				}else{
					echo '<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Error, no se pudo guardar los datos.</div>';
				}
			}
			if(isset($_POST['modificate'])){
				$agente	 			= mysqli_real_escape_string($connect,(strip_tags($_POST["agente"],ENT_QUOTES)));//Escanpando caracteres 
					...
					if($update){
						header("Location: action-edit-order-paid.php?nik=".$nik."&pesan=sukses");
					}else{
						echo '<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Error, no se pudo guardar los datos.</div>';
					}
				}				
				...
				if(isset($_POST['tracking'])){
					...
					$track = $arreglo[1];
					$update = mysqli_query($connect, "UPDATE db_sales SET tracking_number='$track' WHERE id='$nik'") or die(mysqli_error());
					if($update){
						header("Location: action-edit-order-paid.php?nik=".$nik."&pesan=sukses");
					}else{
						echo '<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Error, no se pudo guardar los datos.</div>';
            ...
            <div class="col-lg-4 mx-auto">
              ...
              <div class="auto-form-wrapper">
                <form id="submitUsertForm" method="POST" enctype="multipart/form-data">
                  ...
                  <div class="form-group">
                    <div class="input-group">
                    <textarea type="text" name="noteComentOld" name="noteComentOld" style="height: 50px;" class="form-control" readonly="readonly"><?php echo $row ['coment']; ?></textarea>
						          <div class="input-group-append">
                        <span class="input-group-text">
                          <i class="mdi mdi-check-circle-outline"></i>
                        </span>
				  ...
				  <div class="form-group">
					...
					<div class="form-group">
                    	<div class="input-group">
						<a name="tracking" value="<?php echo $row ['tracking_number']; ?>" class="form-control" href="https://tools.usps.com/go/TrackConfirmAction.action?tLabels=<?php echo $row ['tracking_number']; ?>" target="blank" readonly="readonly"><?php echo $row ['tracking_number']; ?></a>
						</div>
                    </div>
					<div class="form-group">
                    	<div class="input-group">
                  ...
                  <div class="form-group">
                    <div class="input-group">
                    <input type="text" name="aprobate" value="<?php echo $row ['status_approval']; ?>" class="form-control" readonly="readonly">
					            <div class="input-group-append">
                        <span class="input-group-text">
                          <i class="mdi mdi-check-circle-outline"></i>
                        </span>
				  ...
				  <div class="form-group">
                    <div class="input-group">
					<input type="date" name="dateOrderUp" value="<?php echo $row ['date_paid']; ?>" class="form-control" readonly="readonly">
						<div class="input-group-append">
                        	<span class="input-group-text">
                          	<i class="mdi mdi-check-circle-outline"></i>
                        	</span>
                  ...
                  <div class="form-group">
                  <input type="submit" name="etiquetar" class="btn btn-sm btn-success" value="Liquidar">
						...
						<a href="../views/index-table-order-paid.php" class="btn btn-sm btn-info">Regresar</a>		
                  </div>
